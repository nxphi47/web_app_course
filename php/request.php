...
class AccessOrders extends AccessDB {
    ...
    public function denormalize_value($key, $value)
    {
        ...
        if ($key == 'cv2') {
            ...
            // output = "item=item_id#quantity=xxx#comment=xxx@item=item_2_id#quantity=xxx"
            // # separate options
            // @ separate items
            // prohibited symbols: =,#,@
            if ($out == "") {
...
//                $comment = (int) explode("=", $atts[2])[1];
    ...
    public function updateCurrentCart($cart, $user) {
        if ($user == null) {
            return $this->updateCurrentCartNoAccount($cart);
        }
        else {
            return $this->updateCurrentCartWithAccount($cart, $user);
        }
    }

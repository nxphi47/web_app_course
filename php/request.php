...
//		$this->fileUploader = new FileUploader();
    ...
    public function denormalize($key, $value) {
        if (is_numeric($value)) {
            $value = $value + 0;
        } else {
    ...
    public function getAll()
    {
        ...
        foreach ($result as $item) {
            ...
            foreach ($item as $key=>$value) {
                $out[$key] = $this->denormalize($key, $value);
            }
    ...
    public function insert()
    {
        ...
        foreach ($this->attrs as $key => $value) {
            ...
            array_push($values, $this->normalize($key, $value));
        }
    ...
    public function insertMany() {
        ...
        foreach ($this->attrs as $row) {
            ...
            foreach ($row as $key => $value) {
                ...
                array_push($atts, $key);
                array_push($vals, $this->normalize($key, $value));
            }
    ...
    public function updateByConstraint($constraint, $arr)
    {
        ...
        foreach ($arr as $key=>$value) {
            if (in_array($key, $this->allKeys) && $key != $this->idName) {
                array_push($atts, "{$key}={$this->normalize($key, $value)}");
            }
            else {
...
class AccessUsers extends AccessDB
{
    ...
    public function normalize($key, $value)
    {
        ...
        $out = parent::normalize($key, $value); // TODO: Change the autogenerated stub
        return $out;
    }

    public function denormalize($key, $value)
    {
        $out = parent::denormalize($key, $value); // TODO: Change the autogenerated stub
        if ($key == 'password') {
            $out = "";
        }
        return $out;
